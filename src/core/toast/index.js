...
const Toast = (props) => {
    const {title, message, color, onClose} = props
    const hex = {
        success: {
    ...
    return (
        <Styles.container background={hex[color].background} indicator={hex[color].indicator}>
            <Styles.content>
                <Styles.title data-test-id="toastTitle">{title}</Styles.title>
                <Styles.message data-test-id="toastContent">{message}</Styles.message>
            </Styles.content>
            <Styles.icon data-test-id="toastClose" href='' onClick={onClose}>
