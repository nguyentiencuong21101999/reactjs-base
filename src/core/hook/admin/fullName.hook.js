 ...
 const FullNameHook = props => {
   ...
   const onChange = event => {
     try {
       ...
       setError('')
       let temp = value.slice(0, maxLength)
       temp = temp.replace(/\s\s+/g, ' ')
       setPassword(temp)
       Validator.isNotEmpty(name, temp, Localize.getLocalize('LC_FULLNAME_EMPTY'))
       Validator.isFullName(name, temp, Localize.getLocalize('LC_FULLNAME_INVALID'))
