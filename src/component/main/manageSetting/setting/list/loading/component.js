...
class LoadingListSettingComponent extends Component {
  constructor(prop) {
    ...
    this.state = {
      ...
      func: {
        handleGetListSettingSuccess: this.handleGetListSettingSuccess.bind(this),
        handleSystemError: this.handleSystemError.bind(this)
      }
    }
  }
  handleGetListSettingSuccess(response) {
    try {
      Logger.info("LoadingListSettingComponent execute handleGetListSettingSuccess");
      Logger.debug("LoadingListSettingComponent execute handleGetListSettingSuccess receive list", response);
      const { ui, timeout } = this.state;
      const { handleSetData } = this.props
      handleSetData(response)
      timeout.setTimeout(false)
    } catch (e) {
      Logger.error(`LoadingListSettingComponent handleGetListSettingSuccess ${e.toString()}`);
    }
  }
