...
class ListNotifyComponent extends Component {
    ...
    this.state = {
      ...
      ui: {
        ...
        listSearch: [
          ...
          {
            text: Localize.getLocalize("LC_SENT_TO"),
            value: "logs.cif",
          },
          {
  ...
  handleOnChangeStatus(event, value) {
    try {
      ...
      if (value === NotifyEnum.FILTER.ALL) {
        this.handleOnChange('channel', undefined)
      } else {
  ...
  handleCreate(event) {
    try {
      ...
      const { ui } = this.state;
      this.handleRedirectWithState(RouteEnum.PAGE.MANAGE_NOTIFY.CREATE);
    } catch (e) {
