...
const StatusParticipantComponent = (value) => {
  ...
  if (value)
    switch (value) {
      case ChallengeEnum.USER_CHALLENGE_POST_STATUS.JOINED:
        styles.color = "#333333"
        styles.backgroundColor = "#D9D9D9"
        break;
      case ChallengeEnum.USER_CHALLENGE_POST_STATUS.COMPLETE:
      case ChallengeEnum.USER_CHALLENGE_POST_STATUS.RANKED:
        styles.color = "#00A7C6"
        styles.backgroundColor = "#E5FDFF"
        break;
    }
  let temp = !Helper.isEmpty(styles) ? <CustomChip sx={{ height: "36px", padding: "0px 10px 0px 10px", fontWeight: 400, fontSize: "12px" }} size="small" style={styles} label={ChallengeEnum.USER_CHALLENGE_POST_STATUS_PARSE[value]} /> : <Typography>{"-"}</Typography>
  return temp
}
const LoadingListParticipantPostHtml = (props) => {
  ...
  const headerRows = [
    ...
    { value: Localize.getLocalize("LC_RANK") },
...
export default LoadingListParticipantPostHtml;
