  ...
  return (
    <Styles.fill isOpen={open}>
      <Styles.container style={{ display: "block" }} isOpen={open} width={488}>
        <Styles.head>
          <Styles.title data-test-id="popupTitle">{Localize.getLocalize("LC_UPDATE_STATUS")}</Styles.title>
        ...
        <Styles.body style={{ paddingBottom: 0 }}>
          <Styles.wrapper data-test-id="popupContent">
            <Styles.headerTitle>
              <Stack sx={{ marginBottom: "10px", color: "#333333" }}>{Localize.getLocalize("LC_UPDATE_STATUS_CONTENT")}</Stack>
              <Select
                list={ui.listTarget}
                isRow={false}
                ...
                notOutline={false}
                styleFormControl={{ minWidth: "250px", border: "1px red" }}
