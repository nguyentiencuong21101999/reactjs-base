...
import ValueHook from "core/hook/dialog/title.hook";
import {
  ...
  return (
    <Styles.fill isOpen={open}>
      <Styles.container style={{ display: "block" }} isOpen={open} width={488}>
        <Styles.head>
          <Styles.title data-test-id="popupTitle">{Localize.getLocalize("LC_UPDATE_CONFIG")}</Styles.title>
        ...
        <Styles.body style={{ paddingBottom: 0 }}>
          <Styles.wrapper data-test-id="popupContent">
            <Styles.headerTitle>
              <Stack sx={{ marginBottom: "10px", color: "#333333" }}>{Localize.getLocalize("LC_UPDATE_CONFIG_CONTENT")} {dialog.data.content.configId}</Stack>
              {/* <Stack sx ={{padding:"10px",color:"#333333"}} > Giá trị </Stack> */}
              <ValueHook
                name="value"
                value={ui.detailChallenge && ui.detailChallenge.title || ""}
                type="text"
                label={Localize.getLocalize("LC_VALUE")}
