  ...
  return (
    <Styles.fill isOpen={open}>
      <Styles.container style={{ display: "block" }} isOpen={open} width={488}>
        <Styles.head>
          <Styles.title data-test-id="popupTitle">{Localize.getLocalize("LC_EDIT")}</Styles.title>
        ...
        <Styles.body style={{ paddingBottom: 0 }}>
          <Styles.wrapper data-test-id="popupContent">
            <Styles.headerTitle>
              <Stack sx={{ marginBottom: "10px", color: "#333333" }}>{Localize.getLocalize("LC_EDIT_MENU_NAME_CONTENT")}</Stack>
              {/* <Stack sx ={{padding:"10px",color:"#333333"}} > Giá trị </Stack> */}
              <ValueHook
                name="menuSettingName"
                value={ui.detailMenuSetting && ui.detailMenuSetting.menuSettingName || ""}
                type="text"
