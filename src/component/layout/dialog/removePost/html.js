    ...
    return (
        <Styles.fill isOpen={open}>
            <Styles.container isOpen={open} width={488}>
                <Styles.head >
                    <Styles.title data-test-id="popupTitle">{Localize.getLocalize("LC_POPUP_REMOVE_POST_TITLE")}</Styles.title>
                ...
                <Styles.body style={{ paddingBottom: 0 }}>
                    <Styles.wrapper data-test-id="popupContent">
                        <Styles.headerTitle>
                            ...
                            <ReasonHook styleFormControl={{ maxWidth: '488px', boxSizing: 'border-box' }}

                                label={Localize.getLocalize("LC_CONTENT_NOTI")}
                                value={Localize.getLocalize("LC_CONTENT_NOTI_REPORT_DEFAULT")}
                                error={timeout.field === 'all' ? Localize.getLocalize(timeout.message) : ''}
                                disabled={timeout.status} name='reason' onChange={handleOnChange} onRef={handleOnRef}
                                styleFormControl={{ fontSize: "16px", width: "95%", marginBottom: "10px" }} testId="inputReason" testMessageId="msgReason"
                                fullWidth={false} />

                            <Stack sx={{ marginTop: "10px", color: "#333333", width: "445px" }}>{Localize.getLocalize("LC_POPUP_REMOVE_POST_CONTENT")}</Stack>
                        </Styles.headerTitle>
                ...
                <Styles.footer>
                    ...
                    <Button testId="popupConfirm" text={Localize.getLocalize('LC_BUTTON_CONFIRM')} onClick={(e) => { e.preventDefault(); handleSubmit() }} disabled={timeout.status || !ui.isShowBtn} loading={timeout.statusLoading} />
                </Styles.footer>
            </Styles.container>
        </Styles.fill>
    )
